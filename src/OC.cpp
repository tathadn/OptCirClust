...
clustering FOC( std::vector<double> & Data_Points,
                 int width, int K,
                 int First, int Last,
                 int Prev, int Next)

{
  ...
  if(Last >= First)
  {
    ...
    frame_info frame = OC(  width,  K, First,  Last, Prev,  Next, S, J, sum_x, sum_x_sq, Cluster_Border);
                ...
                std::vector< std::vector<int> > & Cluster_Border)
{
...

  {
    ...
    frame_info frame_left = OC( width,  K, First,  Middle_Frame - 1, Prev,  Middle_Frame, S, J, sum_x, sum_x_sq, Cluster_Border);

    frame_info frame_right = OC( width,  K,  Middle_Frame + 1, Last, Middle_Frame, Next, S, J,  sum_x, sum_x_sq, Cluster_Border);
