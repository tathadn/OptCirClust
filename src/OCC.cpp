...
clustering FOCC( std::vector<double> & Data_Points,
                 int width, int K,
                 int First, int Last,
                 int Prev, int Next)

{
  ...
  if(Last >= First)
  {
    ...
    std::vector<std::vector<int> > Cluster_Border ( (Last - First + 1), std::vector<int>(K,0) );
    ...
    frame_info frame = OCC(  width,  K, First,  Last, Prev,  Next, S, J, sum_x, sum_x_sq, Cluster_Border);
...
frame_info OCC( int width, int K,
                int First, int Last,
                int Prev, int Next,
                ...
                std::vector< std::vector<int> > & Cluster_Border)
{
...

  {
    ...
    frame_info frame_left = OCC( width,  K, First,  Middle_Frame - 1, Prev,  Middle_Frame, S, J, sum_x, sum_x_sq, Cluster_Border);

    frame_info frame_right = OCC( width,  K,  Middle_Frame + 1, Last, Middle_Frame, Next, S, J,  sum_x, sum_x_sq, Cluster_Border);
